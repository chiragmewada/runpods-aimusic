...
//
// Basic auth alone does not work here: the app sends its own JWT as
// `Authorization: Bearer ...` on API calls, replacing the browser's Basic
// credentials, so those requests fail the check and the browser re-prompts on
// every call. The Basic challenge is therefore only the way in — on success it
// sets a cookie, and the cookie is what subsequent requests are judged on.
// Cookies ride along on same-origin requests regardless of Authorization.
function authGate(user: string, pass: string) {
  const COOKIE = 'runpod_auth';
  const expectedBasic = Buffer.from('Basic ' + Buffer.from(`${user}:${pass}`).toString('base64'));
  const expectedCookie = createHash('sha256').update(`${user}:${pass}`).digest('hex');

  const equals = (a: Buffer, b: Buffer) => a.length === b.length && timingSafeEqual(a, b);

  return {
    name: 'runpod-auth-gate',
    configureServer(server: any) {
      server.middlewares.use((req: any, res: any, next: any) => {
        const match = String(req.headers.cookie || '').match(/(?:^|;\s*)runpod_auth=([a-f0-9]+)/);
        if (match && equals(Buffer.from(match[1]), Buffer.from(expectedCookie))) return next();

        if (equals(Buffer.from(req.headers.authorization || ''), expectedBasic)) {
          res.setHeader(
            'Set-Cookie',
            `${COOKIE}=${expectedCookie}; Path=/; HttpOnly; SameSite=Lax; Max-Age=604800`
          );
          return next();
        }
...
export default defineConfig(async (env) => {
  ...
  return {
    ...
    plugins: [...(base.plugins || []), authGate(user, pass)],
  };
});
VITECONFIG
